...
router.get("/", async (req,res)=>{   // if we apply GET request then it fetches all the data
    try {
...
router.get("/:id", async (req,res)=>{  //GET request to fetch data by ID
    const {id}=req.params;
    try {
...
router.put("/:id", async (req, res) => {   //update the application
    const { id } = req.params;
